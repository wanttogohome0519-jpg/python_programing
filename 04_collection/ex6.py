...
result = {x: x**2 for x in range(1, 22, 2)}
print(result)
...
print({subject: "합격" if score >= 60 else "불합격" for subject, score in scores.items()})
...
subjects = ["국어", "영어", "수학"]
grades = [90, 80, 100]
print({subject: grade for subject, grade in zip(subjects, grades)})
...
print({item: stock.get(item, 0) + incoming.get(item, 0) for item in set(stock) | set(incoming)})
for item in set(stock) | set(incoming):
    stock[item] = stock.get(item, 0) + incoming.get(item, 0)
dict(stock)
